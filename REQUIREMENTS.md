...
- R3.5 **[done]** Inputs can be saved to and loaded from a JSON file. The file includes the college list in order with carry forwards, percentage, rounding rule, and tie rule (format version 3; version-1 files load with carry forward 0, and files before version 3 load with the eligible-faculty tie rule). The page describes the file's shape and which fields are required in a "Files" section of the method description, linked from beside the buttons, so a file can be prepared by hand or from a spreadsheet; a failed load names the reason and points to that section. A second button in the carry-forward report saves next year's starting file: the college names, next year's carry forwards, and counts cleared.
